...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        Path path = new Path(vertices.get(start)); // Path object to be returned
        PriorityQueue<Path> frontier = new PriorityQueue<Path>(); // holds discovered Paths

        frontier.add(path);

        boolean foundEnd = start.equals(end);
        List<T> foundEnds = new ArrayList<T>();

        while (!foundEnd) {
            if (frontier.isEmpty())
                throw new NoSuchElementException("No path found");

            Path pathToCheck = frontier.poll();

            foundEnds.add(pathToCheck.end.data);

            if (pathToCheck.end.data.equals(end)) {
                path = pathToCheck;
                foundEnd = true;
            } else {
                for (Edge e : pathToCheck.end.edgesLeaving) {
                    Path newPath = new Path(pathToCheck, e);

                    if (!foundEnds.contains(newPath.end.data))
